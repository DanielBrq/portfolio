    ...
    <div :class="[
      ...
      <div class="relative flex flex-col h-64 mobile:h-72 sm:h-80 p-4 sm:p-6 md:p-8 lg:p-10">
        <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
          <h3
            class="text-niel-primary-200 text-lg sm:text-xl md:text-2xl lg:text-3xl font-light uppercase tracking-wider mobile:tracking-widest sm:tracking-[0.15em] font-serif">
            {{ title }}
          </h3>
          ...
          <div class="shrink-0 mt-1">
            <span :class="[
              ...
              <span class="w-1.5 h-1.5 rounded-sm mr-2" :class="props.repositoryPrivate ? 'bg-rose-400' : 'bg-emerald-400'
                "></span>
              {{ repoVisibility }}
            </span>
          </div>
        ...
        <div class="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-6 mt-auto pt-4 sm:pt-6 border-t border-white/5">
          <a v-if="githubUrl" :href="githubUrl" target="_blank"
            ...
            <IconGithub class="size-5 sm:size-6 transition-transform duration-300 group-hover/btn:scale-110" />
            <span class="text-sm sm:text-base font-light">GitHub</span>
          </a>

          <a v-if="websiteUrl" :href="websiteUrl" target="_blank"
            class="group/btn flex items-center gap-2 text-niel-neutral-400 hover:text-[#c5a880] transition-all duration-300">
            <IconUrl class="size-5 sm:size-6 transition-transform duration-300 group-hover/btn:scale-110" />
          ...
          <nuxt-link v-if="to" :to="to"
            ...
            <IconFolder class="size-4 sm:size-5 transition-transform duration-300 group-hover/link:translate-x-1" />
          </nuxt-link>
        </div>
      </div>
    </div>
